...
async def list_user_courses(auth: Auth = Depends()) -> List[Course]:
...
@router.post('', response_model=List[CourseRead], status_code=status.HTTP_201_CREATED)
async def user_enroll_course(
    course_id: int = Body(..., embed=True), auth: Auth = Depends()
) -> List[Course]:
    user = await auth.check_roles(['student', 'teacher'])
    ...
    if user.role == 'student':
        course_user = await crud_course_student.create(
            course_id=course_id, student_id=user.id
        )
        courses = user.students_courses
    else:
        course_user = await crud_course_teacher.create(
