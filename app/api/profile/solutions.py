...
async def list_solutions(auth: Auth = Depends()) -> List[Solution]:
...
@router.put('', response_model=SolutionInfo)
async def publish_solution(
    solution_put: SolutionPut = Body(...), auth: Auth = Depends()
) -> Solution:
    user = await auth.check_roles(['student'])

    module = await crud_module.get_by_number_in_course(
        course_id=solution_put.course_id, number=solution_put.module_number
    )
    if module is None or module.status != 'started':
        raise HTTPException(status.HTTP_400_BAD_REQUEST)

    if module.course_id not in [
        owned_course.id for owned_course in user.students_courses
    ]:
...
@router.get('/{solution_id}', response_model=SolutionInfo)
...
@router.put('/{solution_id}', response_model=SolutionInfo)
