...
async def test_success(client):
    ...
    response = client.post(
        '/api/auth/login', json={'email': db_user.email, 'password': 'student_1'}
    )
...
async def test_fail_wrong_email(client):
    ...
    response = client.post(
        '/api/auth/login',
        json={'email': 'wrong_email@api.edu', 'password': 'student_1'},
    )
...
async def test_fail_wrong_password(client):
    ...
    response = client.post(
        '/api/auth/login', json={'email': db_user.email, 'password': 'wrong_password'}
    )
