...
async def test_publish_solution(client, set_auth, add_student_enroll_course):
    ...
    for i in range(2):
        ...
        assert solution["teacher_id"] == None
        assert solution["review"] == None 
        assert solution["score"] == None
...
async def test_publish_solution_fail_module_not_found(client, set_auth, add_student_enroll_course):
    _, course, module, db_student = add_student_enroll_course
    await crud_module.set_status(module["id"], 'started')
...
async def test_publish_solution_fail_module_status(client, set_auth, add_student_enroll_course, status):
    _, course, module, db_student = add_student_enroll_course
...
async def test_publish_solution_fail_forbidden(client, set_auth, add_teacher_course_module):
...
async def test_info_solution_student(client, set_auth, add_solution):
    ...
    assert solution_info["review"] == None
    assert solution_info["score"] == None
...
def test_publish_review_success(client, set_auth, add_solution):
    db_teacher, course, module, _, solution = add_solution
