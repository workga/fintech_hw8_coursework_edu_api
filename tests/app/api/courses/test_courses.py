from tests.app.api.conftest import add_teacher_model, get_course_create_schema
...
async def test_create_success(client, set_auth):
    ...
    set_auth(db_teacher)
    response = client.post('/api/courses', json=course_create)
...
async def test_get_info_success(client, set_auth):
    ...
    set_auth(db_teacher)
    client.post('/api/courses', json=course_create)
